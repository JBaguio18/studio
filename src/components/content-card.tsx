...
export function ContentCard({ post, className }: ContentCardProps) {
  return (
    <Link href={`/content/${post.id}`} className="group block">
      <Card className={cn("flex h-full flex-col overflow-hidden transition-all group-hover:shadow-lg", className)}>
        {post.image_url && (
        ...
        <CardHeader>
          <p className="font-bold uppercase text-accent">{post.creator_name}</p>
